...
async function testConfigAPI() {
	...
	try {
		...
		if (config.forceUpdate) {
			...
			console.log(
				`    - ${config.forceUpdate.fields.slice(0, 3).join(", ")}...`,
			);
		...
		console.log(
			`  GitHub Token Exchange URL: ${config.oauth.github.tokenExchangeUrl}`,
		);

		console.log("\n模型提供商:");
		console.log(`  总数: ${config.modelProviders.length} 个`);
		console.log(
			`  前 5 个: ${config.modelProviders
				.slice(0, 5)
				.map((p) => p.name)
				.join(", ")}`,
		);
		...
		console.log(
			`  列表: ${config.searchProviders.map((p) => p.name).join(", ")}`,
		);

		console.log("\nMCP 市场源:");
		console.log(`  总数: ${config.mcpMarketSources.length} 个`);
		console.log(
			`  列表: ${config.mcpMarketSources.map((s) => s.name).join(", ")}`,
		);
