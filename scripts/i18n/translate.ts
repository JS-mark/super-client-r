...
const LOCALES_DIR = path.resolve(__dirname, '../../src/renderer/src/i18n/locales');
const BASE_LANG = 'zh';

// Initialize Anthropic client
const apiKey = process.env.ANTHROPIC_API_KEY || process.env.AI_TRANSLATE_KEY || process.env.VITE_ANTHROPIC_API_KEY;
if (!apiKey) {
  console.error(chalk.red('Error: ANTHROPIC_API_KEY, AI_TRANSLATE_KEY, or VITE_ANTHROPIC_API_KEY environment variable is required.'));
  process.exit(1);
}
...
async function translateBatch(texts: Record<string, string>, targetLang: string): Promise<Record<string, string>> {
    if (Object.keys(texts).length === 0) return {};

    console.log(chalk.yellow(`Translating ${Object.keys(texts).length} keys to ${targetLang}...`));
...
async function run() {
  ...
  const languages = fs.readdirSync(LOCALES_DIR).filter(f => f !== BASE_LANG && fs.statSync(path.join(LOCALES_DIR, f)).isDirectory());
  ...
  for (const lang of languages) {
    ...
    for (const file of baseFiles) {
      ...
      if (Object.keys(missingKeys).length > 0) {
        console.log(chalk.yellow(`  File: ${file} - Found ${Object.keys(missingKeys).length} missing keys`));
