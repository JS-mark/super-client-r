...
import type { AgentMessage, AgentSession, AgentStreamEvent } from '../types'
...
export function registerAgentHandlers(): void {
  // 创建会话
  ipcMain.handle(AGENT_CHANNELS.CREATE_SESSION, (_event: IpcMainInvokeEvent, config: AgentConfig) => {
  ...
  // 获取状态
  ipcMain.handle(AGENT_CHANNELS.GET_STATUS, (_event: IpcMainInvokeEvent, sessionId: string) => {
  ...
  // 停止 agent
  ipcMain.handle(AGENT_CHANNELS.STOP_AGENT, (_event: IpcMainInvokeEvent, sessionId: string) => {
  ...
  // 获取会话消息
  ipcMain.handle('agent:get-messages', (_event: IpcMainInvokeEvent, sessionId: string) => {
    const messages = agentService.getMessages(sessionId)
    return { success: true, data: messages }
  })

  // 清除会话消息
  ipcMain.handle('agent:clear-messages', (_event: IpcMainInvokeEvent, sessionId: string) => {
    agentService.clearMessages(sessionId)
    return { success: true }
  })

  // 删除会话
  ipcMain.handle('agent:delete-session', (_event: IpcMainInvokeEvent, sessionId: string) => {
