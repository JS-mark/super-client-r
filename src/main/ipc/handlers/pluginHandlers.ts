...
export const PLUGIN_CHANNELS = {
	...
	GET_MARKDOWN_THEME_CSS: "plugin:getMarkdownThemeCSS",
} as const;
...
`;
	...
	ipcMain.handle(
		...
		async (): Promise<{
			success: boolean;
			data?: { pluginId: string; themeId: string } | null;
			error?: string;
		}> => {
			try {
			...
			try {
				if (pluginId === null) {
					...
					if (!pluginManager.isSkinPlugin(pluginInfo)) {
						return {
							success: false,
							error: `Plugin ${pluginId} is not a skin plugin`,
						};
					}
			...
			try {
				if (pluginId === null) {
					...
					if (!pluginManager.isMarkdownThemePlugin(pluginInfo)) {
						return {
							success: false,
							error: `Plugin ${pluginId} is not a markdown theme plugin`,
						};
					}
					// Ensure plugin is activated
					if (!pluginManager.isPluginActive(pluginId)) {
	...
	ipcMain.handle(
		...
		async (): Promise<{
			...
			try {
				const css = pluginManager.getActiveMarkdownThemeCSS();
