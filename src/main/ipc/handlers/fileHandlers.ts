...
// 获取文件类型
function getFileType(mimeType: string): "image" | "document" | "code" | "audio" | "video" | "archive" | "other" {
...
export function registerFileHandlers() {
	// 选择文件
	ipcMain.handle(FILE_CHANNELS.SELECT_FILES, async (_, options?: {
		multiple?: boolean;
		...
		const result = await dialog.showOpenDialog({
			...
			filters: options?.filters || [
				...
				{ name: "代码", extensions: ["js", "ts", "json", "html", "css", "py"] },
			],
		});
		...
		const files = result.filePaths.map((path) => {
			...
			// 简单的 MIME 类型判断
			if (IMAGE_EXTENSIONS.includes(ext)) mimeType = `image/${ext.replace(".", "").replace("jpg", "jpeg")}`;
	...
	// 读取文件内容
	ipcMain.handle(FILE_CHANNELS.READ_FILE, async (_, filePath: string, options?: { encoding?: BufferEncoding; maxSize?: number }) => {
		try {
	...
	// 保存附件
	ipcMain.handle(FILE_CHANNELS.SAVE_ATTACHMENT, async (_, data: {
		sourcePath: string;
		conversationId?: string;
		messageId?: string;
		customName?: string;
	...
	// 删除附件
	ipcMain.handle(FILE_CHANNELS.DELETE_ATTACHMENT, async (_, attachmentPath: string) => {
		try {
	...
	// 打开附件
	ipcMain.handle(FILE_CHANNELS.OPEN_ATTACHMENT, async (_, attachmentPath: string) => {
		try {
			if (!existsSync(attachmentPath)) {
				return { success: false, error: "文件不存在" };
			}
