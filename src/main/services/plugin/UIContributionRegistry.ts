...
export class UIContributionRegistry extends EventEmitter {
		...
		return {
			dispose: () => {
				this.sidebars = this.sidebars.filter(
					(s) => !(s.pluginId === config.pluginId && s.id === config.id),
				);
				this.broadcastChange();
			},
		};
	 ...
	 * 注册设置面板
	 */
	registerSettingsPanel(config: SettingsPanelContribution): {
		dispose(): void;
	} {
		...
		return {
			dispose: () => {
				this.pages = this.pages.filter(
					(p) => !(p.pluginId === config.pluginId && p.id === config.id),
				);
				this.broadcastChange();
			},
		};
	...
	unregisterAll(pluginId: string): void {
		...
		this.sidebars = this.sidebars.filter((s) => s.pluginId !== pluginId);
		this.settingsPanels = this.settingsPanels.filter(
