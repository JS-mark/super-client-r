...
export class AgentService extends EventEmitter {
  ...
  /**
   * 模拟 agent 响应（实际应该使用 Claude Agent SDK）
   */
  private async simulateAgentResponse(
    ...
    // 这里是模拟实现，实际应该使用 @anthropic-ai/claude-agent-sdk
    // TODO: 集成实际的 Claude Agent SDK
    ...
    const lastContent = typeof lastMsg?.content === 'string'
      ...
      : Array.isArray(lastMsg?.content)
        ? (lastMsg.content[0] as any)?.text || ''
        : ''

    const responseText = `This is a simulated response from the agent. In production, this would use the Claude Agent SDK to process your message: "${lastContent}"`
