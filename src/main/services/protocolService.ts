...
export function parseProtocolUrl(url: string): ProtocolData | null {
	try {
		...
		logger.error("Failed to parse protocol URL", error instanceof Error ? error : undefined);
		return null;
	}
}
...
export async function handleProtocolData(
	...
	logger.info("Handling protocol action", { action: data.action, params: data.params });
...
async function handleImportSkill(
	...
	try {
		...
		logger.info("Skill imported successfully", { name: skillName, path: skillPath });
		...
		logger.error("Failed to import skill", error instanceof Error ? error : undefined);
		mainWindow?.webContents.send("protocol:skill-imported", {
			success: false,
...
async function handleImportConfig(
	...
	try {
		...
		logger.error("Failed to import config", err instanceof Error ? err : new Error(String(err)));
		mainWindow?.webContents.send("protocol:config-imported", {
			success: false,
...
async function handleAuthCallback(
	...
	if (error) {
		logger.error("Auth callback error", new Error(error), { error_description });
