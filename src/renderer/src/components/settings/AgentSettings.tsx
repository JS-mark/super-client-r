...
function SettingRow({
	...
	return (
		...
		>
			...
			<div className="shrink-0 [&_.ant-select-selection-item]:text-right [&_.ant-select-selection-placeholder]:text-right [&_input]:text-right!">
				{children}
			</div>
		</div>
	);
...
export function AgentSettings({ onTabChange }: AgentSettingsProps) {
	...
	const [claudeCodeProvider, setClaudeCodeProvider] =
			...
			.then((resp) => {
				if (resp.success && resp.data) {
					const ccProv = resp.data.find(
						(p: ModelProvider) => p.claudeCodeEnabled && p.enabled && p.apiKey,
					);
							...
							<div className="space-y-4">
								...
								>
									...
									<div className="flex items-center justify-between py-2">
										...
										<div className="flex items-center gap-2">
											<Tag color={sourceTagColor} className="m-0! text-[11px]!">
												{sourceLabel}
											</Tag>
											{apiKeySource === "none" && (
												<Button
								...
								>
									...
									>
										<Select
											...
											showSearch
											placeholder={
												claudeCodeProvider?.claudeCodeModel ||
												"claude-sonnet-4-5"
											}
											className="w-full!"
											size="small"
								...
								>
									...
									>
										<Select
											...
											options={[
												{
													value: "low",
													label: t("agent.effortLow"),
													desc: t("agent.effortLowDesc"),
												},
												{
													value: "medium",
													label: t("agent.effortMedium"),
													desc: t("agent.effortMediumDesc"),
												},
												{
													value: "high",
													label: t("agent.effortHigh"),
													desc: t("agent.effortHighDesc"),
												},
												{
													value: "max",
													label: t("agent.effortMax"),
													desc: t("agent.effortMaxDesc"),
												},
											]}
											optionRender={(option) => (
									...
									>
										<Select
											...
											options={[
												{
													value: "adaptive",
													label: t("agent.thinkingAdaptive"),
													desc: t("agent.thinkingAdaptiveDesc"),
												},
												{
													value: "enabled",
													label: t("agent.thinkingEnabled"),
													desc: t("agent.thinkingEnabledDesc"),
												},
												{
													value: "disabled",
													label: t("agent.thinkingDisabled"),
													desc: t("agent.thinkingDisabledDesc"),
												},
											]}
											optionRender={(option) => (
												<div className="flex items-center justify-between gap-3">
									...
									<SettingRow
										label={t("agent.maxBudgetUsd")}
										hint={t("agent.maxBudgetHint")}
									>
										<InputNumber
											...
											onChange={(val) =>
												persistConfig({
													...config,
													defaultMaxBudgetUsd: val ?? undefined,
												})
											}
											min={0}
											step={0.5}
								...
								>
									...
									<Select
										...
										onChange={(val: AgentSDKPermissionMode) =>
											persistConfig({
												...config,
												defaultPermissionMode: val || undefined,
											})
										}
										allowClear
										...
										options={[
											{
												value: "default",
												label: t("agent.permissionDefault"),
												desc: t("agent.permissionDefaultDesc"),
											},
											{
												value: "acceptEdits",
												label: t("agent.permissionAcceptEdits"),
												desc: t("agent.permissionAcceptEditsDesc"),
											},
											{
												value: "bypassPermissions",
												label: t("agent.permissionBypass"),
												desc: t("agent.permissionBypassDesc"),
											},
											{
												value: "plan",
												label: t("agent.permissionPlan"),
												desc: t("agent.permissionPlanDesc"),
											},
											{
												value: "dontAsk",
												label: t("agent.permissionDontAsk"),
												desc: t("agent.permissionDontAskDesc"),
											},
										]}
										optionRender={(option) => (
								...
								>
									...
									Object.keys(config.customEnvVars).length > 0 ? (
										<div className="space-y-1.5">
											{Object.entries(config.customEnvVars).map(
												([key, value]) => (
													<EnvVarRow
														key={key}
														envKey={key}
			...
			<div className="space-y-3 mt-4">
				...
				<div className="flex gap-3">
					<div className="flex-1">
						<label className="text-[12px] font-medium mb-1 block">
							{t("agent.profileModel")}
						</label>
						<Input
							value={form.model || ""}
							onChange={(e) =>
								setForm((f) => ({ ...f, model: e.target.value || undefined }))
							}
					...
					<div className="w-20">
						<label className="text-[12px] font-medium mb-1 block">
							{t("agent.profileMaxTurns")}
						</label>
						<InputNumber
							value={form.maxTurns}
							onChange={(val) =>
								setForm((f) => ({ ...f, maxTurns: val ?? undefined }))
							}
							min={1}
							max={100}
							placeholder="auto"
				...
				<div>
					<label className="text-[12px] font-medium mb-1 block">
						{t("agent.profileColor")}
					</label>
					<ColorPicker
						value={form.color || "#1677ff"}
						onChange={(color) =>
							setForm((f) => ({ ...f, color: color.toHexString() }))
						}
						size="small"
						presets={[
							{
								...
								<div className="min-w-0 flex-1">
									<div className="flex items-center gap-2">
										...
										{team.isBuiltin && (
											<Tag
												color="blue"
												className="text-[10px]! m-0! leading-none! py-0!"
											>
												{t("agent.builtin")}
											</Tag>
										)}
