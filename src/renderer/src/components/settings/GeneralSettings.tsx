...
import { App, Button, Card, Input, Select, Space, theme } from "antd";
...
export const GeneralSettings: React.FC = () => {
	...
	const handleCheckUpdate = async () => {
		try {
			...
			if (result.error) {
				message.error(
					t("update.error", "Failed to check for updates: {{error}}", {
						ns: "settings",
						error: result.error,
					}),
				);
				...
				message.success(
					t("update.available", "New version {{latest}} is available", {
						ns: "settings",
				...
				message.info(
					t("update.notAvailable", "You are using the latest version", {
						ns: "settings",
					}),
				);
			}
