...
export function NetworkSettings() {
	...
	// 切换日志启用
	const handleLogToggle = useCallback((enabled: boolean) => {
			...
			<SettingSection title={t("network.proxy")} icon={<ApiOutlined />}>
				...
				{proxyConfig.enabled && (
					<>
						...
						<SettingRow label={t("network.auth")} hint={t("network.authHint")}>
							<Switch
								size="small"
							...
							<>
								<SettingRow label={t("network.username")}>
									<Input
										...
										onBlur={(e) => saveProxy({ username: e.target.value })}
										onChange={(e) =>
											setProxyConfig((prev) => ({
												...prev,
						...
						>
							<Input
								...
								onBlur={(e) => saveProxy({ bypassList: e.target.value })}
								onChange={(e) =>
									setProxyConfig((prev) => ({
										...prev,
						...
						<SettingRow label={t("network.testConnection")}>
							...
							>
								{testing ? t("network.testing") : t("network.testConnection")}
							</Button>
						</SettingRow>
					</>
				)}
