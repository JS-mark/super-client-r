...
/**
 * Renderer 端的 ElectronAPI 类型镜像。已迁移到 `electron-api.ts` 共享契约的
 * namespace 通过 `extends ElectronAPIMigrated` 强制对齐 preload。
 */
export interface ElectronAPI extends ElectronAPIMigrated {
  ...
  chat: {
    ...
    getConversationCwd: (id: string) => Promise<IPCResponse<string>>;
    /** @deprecated Use `getConversationCwd`. */
  ...
  workspaceRuntime: {
    ...
    ) => Promise<IPCResponse<{ applied: boolean; reason?: string }>>;
  };
  ...
  pty: {
    ...
    list: () => Promise<
      IPCResponse<
        Array<{ sessionId: string; shell: string; pid: number; cwd: string }>
      >
    >;
    onData: (
  ...
  appConfig: {
    ...
    // project-session-redesign A-7 (S7) — sticky 默认值 for §9.5 picker。
    // `lastKind`：上次选了普通对话还是项目对话；`lastProjectId`：项目对话时上次选的项目 id。
