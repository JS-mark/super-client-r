...
# 直接运行
PORT=9099 RELAY_KEYS=my-secret-key bun relay-server/index.ts
```
...
```bash
# 编译当前平台
bun relay-server/build.ts current

# 编译所有平台
bun relay-server/build.ts

# 产物在 relay-server/dist/ 目录
ls relay-server/dist/
...
上传到服务器后运行：
```bash
chmod +x ./relay-server-linux-x64
PORT=9099 RELAY_KEYS=my-secret-key ./relay-server-linux-x64
```
...
WORKDIR /app
COPY relay-server/index.ts .
EXPOSE 9099
CMD ["bun", "index.ts"]
```

```bash
docker build -t relay-server -f relay-server/Dockerfile .
docker run -d \
  --name relay-server \
  -p 9099:9099 \
  -e PORT=9099 \
