...
| `useAppShortcuts.ts:41` | 侧边栏切换 (Cmd+B) handler 为空（按钮切换可用） | P2 |
| `ChatInputArea.tsx` | 文档功能仅显示 "coming soon" toast | P2 |
| `ChatInputArea.tsx` | 工具栏 quote/prompt/tools/tags/translate 均为 toast 占位 | P2 |
...
| 附件与消息关联 | 未实现 | P2 | 发送消息时 attachmentIds 未写入 metadata |
| 滚动到指定消息 | 未实现 | P3 | `scrollToMessage(messageId)` |
...
1. 快捷键 handler 补全（Cmd+K 搜索、Cmd+B 侧边栏）
