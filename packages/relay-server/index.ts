...
function handleAuth(ws: ServerWebSocket, msg: RelayAuthMessage): void {
	...
	if (role === "controller") {
		...
		console.log(`[Relay] Device "${deviceId}" joined room "${relayKey}"`);
	} else {
...
function handleClose(ws: ServerWebSocket): void {
	...
	if (role === "controller" && room.controller === ws) {
		...
		if (room.devices.get(deviceId) === ws) {
			...
			console.log(`[Relay] Device "${deviceId}" left room "${relayKey}"`);
...
if (ALLOWED_KEYS) {
	console.log(`[Relay] Allowed keys: ${Array.from(ALLOWED_KEYS).join(", ")}`);
} else {
