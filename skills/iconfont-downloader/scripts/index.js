 ...
 */
async function detectBrowserTool() {
    // 1. 首先检测 MCP 浏览器工具
    try {
...
async function listMcpTools() {
    ...
    if (typeof globalThis !== 'undefined' && globalThis.mcp) {
        return globalThis.mcp.listTools();
    }
    return [];
}
/**
 * 调用 MCP 浏览器工具
 */
async function callMcpBrowserTool(action, params) {
    if (typeof globalThis !== 'undefined' && globalThis.mcp) {
        return globalThis.mcp.callTool('browser-' + action, params);
    }
 ...
 * 使用 API 搜索图标
 */
async function searchByAPI(keyword, limit, page) {
    ...
    return (data.data?.icons || []).map((icon) => ({
        id: icon.id?.toString(),
        name: icon.name,
 ...
 */
async function searchByScraping(keyword, limit, page) {
    if (!currentBrowser) {
    ...
    try {
        ...
        // 提取图标信息
        const icons = await browser.evaluate((maxResults) => {
...
async function download(input) {
    try {
        ...
        // 确定保存路径
        const defaultOutputPath = path.join(process.cwd(), 'src', 'renderer', 'src', 'components', 'icons');
 ...
 * 从详情页获取 SVG
 */
async function fetchSVGFromDetailPage(iconId) {
    // 尝试使用 API 获取
 ...
 * 解析用户的选择
 */
function parseUserSelection(selections, keyword) {
